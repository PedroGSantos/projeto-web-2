...
function addWatchedMovie(email){
    ...
    fetch(`http://www.omdbapi.com/?t=` + movieInput.value + `&apikey=` + omdbApiKey)
...
function updateWatchedMovies(email, movieTitle, movieId){
    ...
    fetch('../assets/data/users_db.json').then((response) => {
    ...
    fetch('../assets/data/users_db.json').then((response) => {
        response.json().then((dados) => {
            ...
            watchedMovies.map((movie) => {
                fetch(`http://www.omdbapi.com/?i=` + movie.id + `&apikey=` + omdbApiKey)
                .then(response => response.json())
                .then(movieData => {
